...
export const useAuthStore = defineStore('auth', () => {
    ...
    const uploadImageToSupabase = async (file) => {
        try {
            ...
            const sanitizeFileName = (originalName) => {
                ...
                // 영문, 숫자, 하이픈, 언더스코어만 사용
                return `profile_${timestamp}_${randomString}.${fileExt}`;
            };

            const fileName = sanitizeFileName(file.name);
            console.log("생성된 안전한 파일명:", fileName);

            // Supabase Storage에 업로드 (폴더 구조 포함)
    ...
    const updateProfile = async (profileDataToUpdate) => {
        console.log("updateProfile 호출됨, 받은 데이터:", profileDataToUpdate);
        ...
        try {
            console.log("Django API 호출 시작, 전송 데이터:", profileDataToUpdate);

            const response = await axios.put(
                `${API_URL.value}/api/v1/accounts/profile/`,
                profileDataToUpdate, // Vue에서 이미 처리된 데이터 (이미지 URL 포함)
                {headers}
            );

            console.log("Django API 응답 성공:", response.data);

            // 로컬 상태 업데이트
            userProfile.value = response.data;
            localStorage.setItem('userProfile', JSON.stringify(response.data));

            console.log("userProfile 업데이트 완료:", userProfile.value);

            return true;

        } catch (error) {
            console.error("Django API 호출 실패:", error);
            ...
            // Vue 컴포넌트에서 에러 처리할 수 있도록 에러를 다시 throw
            throw error;
        }
    };
